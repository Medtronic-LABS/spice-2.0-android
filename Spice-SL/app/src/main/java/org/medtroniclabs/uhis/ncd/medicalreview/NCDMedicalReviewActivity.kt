...
import org.medtroniclabs.uhis.databinding.ActivityNcdMrBaseBinding
...
import org.medtroniclabs.uhis.ncd.assessment.ui.AssessmentReadingActivity
...
import org.medtroniclabs.uhis.ncd.data.Answer
import org.medtroniclabs.uhis.ncd.data.BadgeNotificationModel
import org.medtroniclabs.uhis.ncd.data.Chip
import org.medtroniclabs.uhis.ncd.data.ContinuousMedicalReview
import org.medtroniclabs.uhis.ncd.data.CurrentMedications
import org.medtroniclabs.uhis.ncd.data.InitialMedicalReview
...
import org.medtroniclabs.uhis.ncd.medicalreview.NCDMRUtil.getTypeForDiagnoses
import org.medtroniclabs.uhis.ncd.medicalreview.dialog.NCDDiagnosisDialogFragment
import org.medtroniclabs.uhis.ncd.medicalreview.dialog.NCDMRAlertDialog
import org.medtroniclabs.uhis.ncd.medicalreview.dialog.NCDMentalHealthFragment
import org.medtroniclabs.uhis.ncd.medicalreview.dialog.NCDPatientHistoryDialog
import org.medtroniclabs.uhis.ncd.medicalreview.dialog.NCDPregnancyDialog
import org.medtroniclabs.uhis.ncd.medicalreview.dialog.NCDTreatmentPlanDialog
import org.medtroniclabs.uhis.ncd.medicalreview.fragment.NCDChiefComplaintsFragment
import org.medtroniclabs.uhis.ncd.medicalreview.fragment.NCDClinicalNotesFragment
import org.medtroniclabs.uhis.ncd.medicalreview.fragment.NCDComorbiditiesFragment
import org.medtroniclabs.uhis.ncd.medicalreview.fragment.NCDComplicationsFragment
import org.medtroniclabs.uhis.ncd.medicalreview.fragment.NCDCurrentMedicationFragment
import org.medtroniclabs.uhis.ncd.medicalreview.fragment.NCDLifestyleAssessmentFragment
import org.medtroniclabs.uhis.ncd.medicalreview.fragment.NCDMedicalReviewDiagnosisCardFragment
import org.medtroniclabs.uhis.ncd.medicalreview.fragment.NCDMedicalReviewSummaryFragment
import org.medtroniclabs.uhis.ncd.medicalreview.fragment.NCDObstetricExaminationFragment
...
import org.medtroniclabs.uhis.ncd.medicalreview.viewmodel.NCDComplicationsViewModel
import org.medtroniclabs.uhis.ncd.medicalreview.viewmodel.NCDCurrentMedicationViewModel
import org.medtroniclabs.uhis.ncd.medicalreview.viewmodel.NCDLifestyleAssessmentViewModel
import org.medtroniclabs.uhis.ncd.medicalreview.viewmodel.NCDMedicalReviewDiagnosisCardViewModel
import org.medtroniclabs.uhis.ncd.medicalreview.viewmodel.NCDMedicalReviewSummaryViewModel
...
class NCDMedicalReviewActivity :
    ...
    private fun checkPatientDetails(patientReference: String?) {
        if (viewModel.isPatientStatusCompleted) {
            ...
            if (patientReference.isNullOrBlank()) {
                ...
                viewModel.ncdPatientDiagnosisStatus(
    ...
    private fun summarySuccessDialog() {
        ...
        if (dialog == null) {
            MedicalReviewSuccessDialogFragment
                .newInstance(patientDetailViewModel.isPatientEnrolled())
                .show(
                    supportFragmentManager,
    ...
    private fun medicalReviewReference(): String? =
        viewModel.createMedicalReview.value
            ?.data
            ?.encounterReference
    ...
    private fun handleValidation(): Boolean {
        ...
        patientDetailViewModel.recentBP().let {
            if (it.isBlank()) {
                showErrorDialog(
                    getString(R.string.bp_mandatory_warning),
                    cfText = getString(R.string.add_new_reading),
                    showConfirm = true,
        ...
        val diagnosisList =
            medicalReviewDiagnosisCardViewModel.getConfirmDiagonsis.value
                ?.data
                ?.diagnosis
    ...
    private fun getInitialMedicalReviewData() {
                        ...
                        medications = currentMedicationViewModel.chips.mapNotNull { chip ->
                            Chip(
                                ...
                                value = if (chip.name.equals(
                                    ...
                                    currentMedicationViewModel.comments
                                        .trim()
                ...
                comorbidities = comorbiditiesViewModel.chips.map { chip ->
                    Chip(
                        ...
                        value = if (chip.name.equals(
                            ...
                            comorbiditiesViewModel.comments
                                .trim()
                                .takeIf { it.isNotBlank() }
                        } else {
                            chip.value
                ...
                complications = complicationsViewModel.chips.map { chip ->
                    Chip(
                        ...
                        value = if (chip.name.equals(
                            ...
                            complicationsViewModel.comments
                                .trim()
                                .takeIf { it.isNotBlank() }
                        } else {
                            chip.value
        ...
        withNetworkCheck(connectivityManager, onNetworkAvailable = {
            val initialMr = if (patientDetailViewModel.getNCDInitialMedicalReview()) {
                AnalyticsDefinedParams.NCDContinuousMedicalReviewCreation
            } else {
    ...
    private fun showPregnancyDialog() {
        withNetworkAvailability(online = {
            val hasNCD =
                !(
                    viewModel.ncdPatientDiagnosisStatus.value
                        ?.data
                        ?.get(NCDMRUtil.NCDPatientStatus) as? Map<*, *>
                ).isNullOrEmpty()
        ...
        withNetworkAvailability(online = {
            val hasMentalHealth =
                !(
                    viewModel.ncdPatientDiagnosisStatus.value
                        ?.data
                        ?.get(NCDMRUtil.MentalHealthStatus) as? Map<*, *>
                ).isNullOrEmpty()
            val hasNCD =
                !(
                    viewModel.ncdPatientDiagnosisStatus.value
