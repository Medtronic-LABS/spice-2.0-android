...
import org.medtroniclabs.uhis.databinding.ActivityAboveFiveYearsBaseBinding
...
import org.medtroniclabs.uhis.ui.medicalreview.utils.MedicalReviewDefinedParams.SE_ITEM
import org.medtroniclabs.uhis.ui.medicalreview.utils.MedicalReviewDefinedParams.SUMMARY_ITEM
...
@AndroidEntryPoint
class AboveFiveYearsBaseActivity :
    BaseActivity(),
        ...
        showErrorDialogue(
            ...
            if (isPositive) {
                supportFragmentManager
                    .findFragmentById(R.id.presentingComplaintsContainer)
                    .let { currentFragment ->
    ...
    override fun onClick(view: View) {
        when (view.id) {
            ...
            binding.btnDone.id -> {
                withLocationCheck({
                    patientViewModel.patientDetailsLiveData.value?.data?.let { details ->
                        viewModel.aboveFiveYearsCreateResponse.value
                            ?.data
                            ?.let { response ->
                                if (connectivityManager.isNetworkAvailable()) {
