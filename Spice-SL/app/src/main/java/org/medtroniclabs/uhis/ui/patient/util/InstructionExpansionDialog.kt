...
import org.medtroniclabs.uhis.databinding.DialogInstructionExpansionBinding
import org.medtroniclabs.uhis.formgeneration.extension.customGetParcelable
    ...
    override fun onClick(view: View) {
        when (view.id) {
            binding.btnDone.id -> {
                ...
                model?.instructionEntered = instructionText.toString()
                model?.isInstructionUpdated = true
                prescriptionViewModel.reloadInstruction.value = true
                nurseViewModel.instrctionEntered = instructionText.toString()
                dismiss()
            }
            binding.ivClose.id -> {
                dismiss()
